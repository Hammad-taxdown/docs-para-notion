// Puerta del prompt v11 · 20/08/2026
...
// nacidos de la conversacion real de hoy 10:38-11:01. `node docs/test-prompt-v11.js`.
const fs = require('fs');
...
const p   = fs.readFileSync(V11, 'utf8');   // el que se mide
const v10 = fs.readFileSync(V10, 'utf8');   // el anterior, para los deltas
...
comp('el tamano es coherente (no se ha truncado)', p.length > v10.length && p.length < v10.length + 6000,
     `v10=${v10.length} v11=${p.length}`);
...
comp('P3 · dice explicitamente que NO formule la pregunta y pase a PF8',
     /NO formules la pregunta[\s\S]{0,200}pasa directamente a PF8/.test(p));
...
console.log(`\n${ok} verdes, ${mal} rojas`);
console.log(`v11 = ${car(p)} caracteres (v10 = ${car(v10)}, ${car(p) - car(v10) >= 0 ? '+' : ''}${car(p) - car(v10)})`);
