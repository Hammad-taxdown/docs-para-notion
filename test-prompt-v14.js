// Puerta del prompt v14 · 26/08/2026
// HEREDA las 103 comprobaciones de test-prompt-v13.js, pero medidas SOBRE EL v14:
...
// El cambio del v14 es UNO: el SLA de 24-48 horas del escalado a una persona,
// decidido por el usuario el 26/08 (M6). OJO AL MATIZ QUE MIDEN LAS COMPROBACIONES
// NUEVAS: el plazo YA ESTABA en el prompt cuatro veces, pero para otra cosa -- 'el
// equipo REVISA EL EXPEDIENTE en 24-48 horas'. Lo que faltaba era el plazo de
// RESPUESTA cuando se remite a una persona. Son dos promesas distintas con la misma
// cifra, y la puerta las cuenta por separado para que el dia que una cambie salte.
// `node docs/test-prompt-v14.js`.
const fs = require('fs');
...
const p   = fs.readFileSync(V14, 'utf8');   // el que se mide
const v13 = fs.readFileSync(V13, 'utf8');   // el anterior inmediato
...
// ── 6 · v14 · EL SLA DEL ESCALADO (M6, decidido el 26/08) ────────────────────
...
// LAS DOS PROMESAS SE CUENTAN APARTE. Misma cifra, significados distintos:
// 'revisa el expediente' es cuanto tarda el equipo en mirar los documentos;
// 'responde' es cuanto tarda una persona en contestar un escalado. Si un dia
// una de las dos cambia, esto salta y obliga a distinguirlas en el texto.
comp('SLA · sigue habiendo 4 promesas de REVISION del expediente, ni una mas',
     ...
     `el v14 tiene ${cuenta(p, 'en 24-48 horas')} y el v13 ${cuenta(v13, 'en 24-48 horas')}: esperado +2`);
comp('SLA · no se ha colado ningun otro plazo inventado',
     !/(72|24) ?h(oras)? h[aá]biles/.test(p) && !/en un plazo de \d+ d[ií]as/.test(p));

console.log(`\n${ok} verdes, ${mal} rojas`);
console.log(`v14 = ${car(p)} caracteres (v13 = ${car(v13)}, ${car(p) - car(v13) >= 0 ? '+' : ''}${car(p) - car(v13)})`);
