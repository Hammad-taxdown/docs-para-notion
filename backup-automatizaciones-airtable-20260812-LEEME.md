> **APLANADO EL 26/08/2026.** Estos cinco ficheros vivían en `docs/backup-automatizaciones-20260812/`
> y por eso **nunca se subieron al remoto**: `push-cierre.sh` copia `docs/` con `-maxdepth 1` y el
> subdirectorio se ignoraba **en silencio, con el push en verde**. Se descubrió al añadirle una
> guarda que aborta si aparece un subdirectorio nuevo. Que esto importe y no sea papeleo: son los
> `customScript` de las automatizaciones de Airtable, **los que no se pueden editar ni leer por
> API** (`readOnlyNodeType`), así que este backup era el único sitio del mundo donde existía su
> código — y estaba en un solo portátil.
