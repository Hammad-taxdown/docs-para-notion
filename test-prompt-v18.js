// Puerta del prompt v18 · 04/09/2026 tarde (HEREDA las 281 del v17 medidas sobre el v18, mas las nuevas)
//
// EL CAMBIO DEL v18: (A) la tool transferir_humano (subworkflow mobility_transferir_humano de Iciar):
// el prompt decia en TRES sitios que el bot no puede transferir, y ahora si, por la herramienta. (B) Las
// mejoras de la pagina de Notion «Mejoras» (Alina/Iciar, 04/09) que son de prompt: 13 parches.
//
// TRECE COMPROBACIONES HEREDADAS SE RE-BASELINAN A PROPOSITO, Y SE DICE AQUI:
//   (a) el enlace de Calendly cambia (link Calendly, C4): el nuevo 1 vez, el viejo 0.
//   (b) la bienvenida cambia entera (N1 x3): se anclan las frases nuevas y se comprueba que el v17 tenia las viejas.
//   (c) NIVEL 2 (SLA): la frase pasa de «escribenos a support» a «te paso con una persona del equipo»; el 24-48 se queda.
//   (d) CINCO -> SEIS herramientas (N4 x2, V17-3).
//   (e) los recuentos de «expediente completo» (P4+P5, V16-5): +1 por la excepcion del certificado de la empresa.
//   (f) las dos ventanas de tamano (V16-7, V17-4): se miden contra el v17, +6.000..7.000 (18 parches).
//   (g) 'CASO COMPLEJO conserva sus cinco señales': el declarante foral deja de ser señal (decision del usuario 04/09).
// Ninguna otra comprobacion se toca. `p` es SIEMPRE el v18.
//
// ── LO QUE SIGUE ES LA CABECERA HEREDADA DEL v17 ──────────────────────────────
//
...
const V18 = path.join(__dirname, 'prompt-final-2026-09-04-v18.txt');
...
const p   = fs.readFileSync(V18, 'utf8');   // EL QUE SE MIDE
const v17 = fs.readFileSync(V17, 'utf8');   // el anterior inmediato
...
comp('CASO COMPLEJO conserva cuatro señales + «otras particularidades» (RE-BASELINE v18: el declarante foral DEJA de ser señal)',
     ['Vía de acceso distinta','No dispone de la carta','Salario no definido','El cónyuge también quiere acogerse','Otras particularidades fiscales']
       .every(x => casoComplejo.includes(x)) && !casoComplejo.includes('- Declarante foral'));
...
for (const [nombre, url] of Object.entries(LINKS)) {
  if (nombre === 'Calendly (ya estaba)') {
    // RE-BASELINE v18 (a): el enlace de Calendly CAMBIA (Notion 04/09). El nuevo una vez, el viejo cero.
    comp(`link ${nombre}: el NUEVO exactamente 1 vez y el viejo 0 (RE-BASELINE v18)`,
         cuenta(p, 'https://calendly.com/taxdown-espana/movilidad-internacional-slot-clon') === 1 && cuenta(p, url) === 0 && cuenta(v17, url) === 1,
         'nuevo: ' + cuenta(p, 'https://calendly.com/taxdown-espana/movilidad-internacional-slot-clon') + ', viejo: ' + cuenta(p, url));
    continue;
  }
...
comp('P4+P5 · «expediente completo»: v17+1 (RE-BASELINE v18: la excepcion del certificado de la empresa en el CIERRE)',
     cuenta(p, 'expediente completo') === cuenta(v17, 'expediente completo') + 1, 'v18=' + cuenta(p, 'expediente completo') + ' v17=' + cuenta(v17, 'expediente completo'));
...
comp('C4 · el enlace de Calendly es UNO, el nuevo de taxdown-espana (RE-BASELINE v18: cambio el 04/09)',
     cuenta(p, 'https://calendly.com/taxdown-espana/movilidad-internacional-slot-clon') === 1 && !p.includes('calendly.com/d/csbw'));
...
comp('SLA · el NIVEL 2 del escalado tambien lo dice (RE-BASELINE v18: ahora transfiere, y mantiene el 24-48)',
     /Te paso con una persona del equipo, que sigue desde aquí con todo lo que hemos hablado; normalmente responde en 24-48 horas\." Y llama a transferir_humano/.test(p) &&
     /lo coge desde aquí, normalmente en 24-48 horas/.test(v17));
...
comp('N1 · la bienvenida es la NUEVA en humano (RE-BASELINE v18: Notion 04/09), y la vieja ya no esta',
     p.includes('"¡Hola! Soy el asistente del equipo Mobility de TaxDown. Puedo resolver tus dudas sobre la Ley Beckham') &&
     !p.includes('Pero... ¿Qué es?') && v17.includes('Pero... ¿Qué es?'));
comp('N1 · con el resumen del regimen en humano (RE-BASELINE v18): 24% fijo sobre el trabajo y lo de fuera no tributa',
     p.includes('Durante 6 años pagas un 24%\nfijo sobre tus ingresos del trabajo, y el resto de tus rentas solo tributan aquí si son de origen español') &&
     p.includes('no tributa en España mientras estés en el régimen.'));
comp('N1 · y con los dos requisitos literales; la linea del departamento Mobility se va (RE-BASELINE v18)',
     p.includes('no debes haber sido residente fiscal en España en los 5 años anteriores y tienes 6 meses desde el alta en la') &&
     !p.includes('En el departamento Mobility revisamos tu caso') && v17.includes('En el departamento Mobility revisamos tu caso'));
...
comp('N4 · la regla 10 dice SEIS herramientas (RE-BASELINE v18: eran CINCO), y ya no TRES',
     /10\. TIENES SEIS HERRAMIENTAS Y DEBES USARLAS/.test(p) && !/TIENES CINCO HERRAMIENTAS/.test(p) && !/TIENES CUATRO HERRAMIENTAS/.test(p) &&
     !/TIENES TRES HERRAMIENTAS/.test(p) &&
     !/Tienes TRES herramientas/.test(p));
comp('N4 · y el recordatorio final tambien dice SEIS (RE-BASELINE v18) y nombra enviar_autorizacion y transferir_humano',
     /^7\. Tienes SEIS herramientas y hay que usarlas: leer_expediente una vez al empezar, calcular_plazo/m.test(p) &&
     /enviar_autorizacion una vez, en el paso de la autorización de TaxDown, y transferir_humano solo cuando el cliente pida una persona/.test(p));
...
comp('V16-5 · «expediente completo» aparece v15+2 (RE-BASELINE v18: +1 por la excepcion del certificado), ni una mas',
     cuenta(p, 'expediente completo') === cuenta(v15, 'expediente completo') + 2, 'v18=' + cuenta(p, 'expediente completo') + ' v15=' + cuenta(v15, 'expediente completo'));
...
comp('V16-7 · (RE-BASELINE v18) el v18 crece entre 6.000 y 7.000 caracteres sobre el v17 (caza un fichero truncado o inflado; 17 parches)',
     car(p) - car(v17) >= 6000 && car(p) - car(v17) <= 7000, `+${car(p) - car(v17)}`);
...
comp('V17-3 · ni CUATRO ni CINCO herramientas aparecen ya (RE-BASELINE v18: SEIS x2)',
     !/CUATRO HERRAMIENTAS/.test(p) && !/CUATRO herramientas/.test(p) && !/CINCO HERRAMIENTAS/.test(p) && !/CINCO herramientas/.test(p) && cuenta(p, 'SEIS') === 2);
...
comp('V17-4 · (RE-BASELINE v18) el v17 local sigue midiendo +900..1.500 sobre el v16 (el fichero anterior no se ha tocado)',
     car(v17) - car(v16) >= 900 && car(v17) - car(v16) <= 1500, `+${car(v17) - car(v16)}`);


// ══════════════════════════════════════════════════════════════════════════════
// PARTE V18 · LA TOOL transferir_humano Y LAS MEJORAS DE NOTION (04/09/2026 tarde)
// ══════════════════════════════════════════════════════════════════════════════
// Anclado en la LINEA OPERATIVA y contando apariciones, como manda la regla del 31/08.
// A · transferir_humano
comp('V18-A1 · la ficha de la tool esta en la regla 10, detras de enviar_autorizacion, con sus 4 casos',
     /sigue con el siguiente documento\.\n- transferir_humano: pasa la conversación a una persona del equipo de Mobility/.test(p) &&
     /\(1\) el cliente pide hablar con una persona, un agente, un humano o un responsable, aunque sea una sola vez; \(2\) está enfadado o frustrado de forma repetida/.test(p) &&
     /\(4\) no puede conseguir el certificado de su empresa y la conversación se queda bloqueada/.test(p));
comp('V18-A1 · la ficha manda avisar ANTES, llamar con el motivo y NO seguir despues',
     /ANTES de llamarla manda UN mensaje corto avisando/.test(p) && /DESPUÉS NO SIGAS con el flujo/.test(p) &&
     /NUNCA digas que has transferido si no la has llamado y te ha devuelto ok:true/.test(p));
comp('V18-A2 · la prohibicion de transferir DESAPARECE de las tres frases (y estaba en el v17)',
     !p.includes('no puedes transferir a nadie') && !p.includes('ni transferir a una persona') && !p.includes('agendar llamadas, ni validar documentos, ni transferir') &&
     v17.includes('no puedes transferir a nadie') && v17.includes('ni transferir a una persona'));
comp('V18-A2 · la regla 10 dice que transferir SI, pero UNICAMENTE con la herramienta',
     /Transferir a una persona SÍ puedes, pero ÚNICAMENTE con la herramienta transferir_humano y en los casos de su ficha/.test(p));
comp('V18-A3 · NIVEL 3 tambien transfiere', /manda ese mismo aviso y llama a transferir_humano\. Sin ofrecer alternativas ni seguir preguntando/.test(p));
comp('V18-A3 · PIDE HABLAR CON UN HUMANO: basta UNA vez, aviso y tool con motivo',
     /PIDE HABLAR CON UN HUMANO \(una persona, un agente, un asesor, un responsable; basta con que lo pida UNA vez\):/.test(p) &&
     /Y llama a transferir_humano con el motivo "pide hablar con una persona"\./.test(p) &&
     !p.includes('"Por supuesto. Escríbenos a support@taxdown.es y una persona del equipo te atiende.'));
comp('V18-A3 · el segundo empujon del modo preguntas ofrece una persona por la tool, no el correo',
     /además hablar con una persona del equipo \(si acepta, llama a transferir_humano\)/.test(p) && !p.includes('además hablar con una persona en support@taxdown.es'));
comp('V18-A4 · transferir_humano aparece EXACTAMENTE 8 veces (ficha x2, regla 10, N2, N3, aviso, humano, empujon, resumen)',
     cuenta(p, 'transferir_humano') === 8, 'apariciones: ' + cuenta(p, 'transferir_humano'));
comp('V18-A4 · el v17 no la nombraba (el cambio es real)', !v17.includes('transferir_humano'));
comp('V18-A4 · las seis tools siguen nombradas',
     ['leer_expediente', 'calcular_plazo', 'guardar_datos_cliente', 'analizar_documento', 'enviar_autorizacion', 'transferir_humano'].every(t => p.includes(t)));
// B · Notion
comp('V18-B5 · la bienvenida se presenta y dice lo que puede hacer (dudas, requisitos, datos y documentos)',
     /Soy el asistente del equipo Mobility de TaxDown\. Puedo resolver tus dudas sobre la Ley Beckham, comprobar contigo si\ncumples los requisitos y, si encaja, recoger los datos y documentos/.test(p));
comp('V18-B5 · la bienvenida sigue terminando con la pregunta de arranque y sin «Pero... ¿Qué es?»',
     p.includes('¿Comprobamos si cumples los requisitos?"') && cuenta(p, 'Pero... ¿Qué es?') === 0);
comp('V18-B5 · las cuatro opciones van en LISTA NUMERADA con «Elige una opción para continuar:»',
     /Preséntalas como una LISTA NUMERADA, una opción por línea, precedida de "Elige una opción para continuar:"/.test(p));
comp('V18-B6 · arranque directo: si ya pide comprobar, «Por supuesto, vamos a hacerte unas preguntas...» + contexto + fecha de llegada',
     /Por supuesto, vamos a hacerte unas preguntas para ver\nsi cumples los requisitos\. Antes de nada, para ponerte en contexto:/.test(p) &&
     /Para empezar, ¿en qué fecha exacta llegaste a España\? \(día\/mes\/año\)"\. Si escribe "tengo una duda", contéstala directamente\./.test(p));
comp('V18-B7 · las rentas de origen espanol se explican en humano y se prohibe la frase de «como no residente»',
     /nunca digas "las rentas de fuente española tributan como no residente"/.test(p) &&
     /Lo que tengas fuera de España, como inversiones, inmuebles o cuentas en otro país, no tributa aquí mientras estés en el régimen/.test(p));
comp('V18-B8 · el salario tiene frase modelo en humano con los 60.000 y la llamada con asesores de movilidad internacional',
     /suele salir favorable a partir de unos 60\.000 € brutos anuales\. Si estás por debajo, lo más conveniente es que agendes una llamada con nuestro equipo de asesores de movilidad internacional/.test(p) &&
     /No le digas nunca que no le compensa\./.test(p));
comp('V18-B9 · FILTRO A: consejo de conservar certificados de residencia fiscal, sin ser descarte ni complejidad',
     /te conviene conservar los certificados de residencia fiscal de los países donde has vivido esos años/.test(p) &&
     /No es un descarte ni una señal de complejidad: es un consejo, y sigues\./.test(p));
comp('V18-B10 · D2 ya no exige el prefijo ni el «+» (y el v17 si)',
     /- D2 — Teléfono: "¿Cuál es tu número de teléfono\? Si no es español, ponme también el prefijo del país\." NO le hagas repetirlo por no haber puesto el "\+"/.test(p) &&
     !p.includes('prefijo de país obligatorio') && v17.includes('prefijo de país obligatorio'));
comp('V18-B10 · VALIDACION: el telefono lo valida el sistema, con o sin +34',
     /- Teléfono: un número español de 9 cifras vale con o sin \+34 \(el sistema lo normaliza\)/.test(p) && !p.includes('Rechazar si falta el prefijo'));
comp('V18-B11 · D5 pide calle, numero, planta, puerta, CP y municipio, y planta/puerta van en sus parametros',
     /Necesito la calle y el número, la planta y la puerta si las tiene, el código postal y el municipio\."/.test(p) &&
     /PLANTA Y PUERTA VAN EN SUS PARÁMETROS \(`planta`, `puerta`\)/.test(p));
comp('V18-B12 · varios datos en un mensaje: todos, en UNA llamada, sin repreguntar',
     /PERO SI EL CLIENTE TE DA VARIOS DATOS EN UN MISMO MENSAJE/.test(p) && /mándalos juntos en UNA sola llamada a guardar_datos_cliente/.test(p));
comp('V18-B13 · lo contado en el modo preguntas no se repregunta: se confirma en una linea',
     /LO QUE TE HAYA CONTADO MIENTRAS PREGUNTABA, CUENTA\./.test(p) && /confírmalo en una línea/.test(p));
comp('V18-B14 · certificado de la empresa: firmado por la EMPRESA, NUNCA por TaxDown, con la plantilla UNA vez',
     /firmado por la EMPRESA \(normalmente RRHH\), NUNCA por TaxDown/.test(p) &&
     cuenta(p, 'https://cdn.prod.website-files.com/6978bfbe89b459a3e1a62fcf/6a2a765e6ea2391205f0c655_Certificado-Generico-Empresas.docx') === 1);
comp('V18-B14 · si no lo tiene NO se bloquea: sigue con los demas documentos y cierra igual',
     /SI NO LO TIENE TODAVÍA, NO BLOQUEES LA CONVERSACIÓN/.test(p) && /SIGUE con los demás documentos/.test(p));
comp('V18-B14 · CIERRE: unica excepcion al «todo recogido» es la autorizacion de la empresa',
     /ÚNICA EXCEPCIÓN: la autorización de la empresa\. Si es LO ÚNICO que falta \(la está pidiendo a RRHH\), cierra como "expediente completo" igual/.test(p));
comp('V18-B14 · pero el NIF sigue siendo obligatorio para cerrar (la excepcion no lo toca)',
     /Y HAY NIF O NIE GUARDADO: con solo un pasaporte el expediente NO está completo/.test(p));
comp('V18-B15 · el Calendly nuevo va en el mensaje del caso complejo y sigue siendo el UNICO enlace de agendar',
     p.includes('una llamada con uno de nuestros fiscales, para asegurarnos de tramitarlo bien. Puedes elegir el momento que mejor te venga aquí: https://calendly.com/taxdown-espana/movilidad-internacional-slot-clon\n') &&
     /Ese enlace es el ÚNICO que puedes dar para agendar/.test(p));
comp('V18-B16 · paternidad proactiva: tambien si solo cuenta que va a ser padre, y nunca «no afecta»',
     /Y TAMBIÉN SI SOLO TE CUENTA QUE VA A SER PADRE O MADRE sin preguntar por impuestos/.test(p) && /nunca contestes que "no afecta"/.test(p));
comp('V18-B17 · un mensaje ambiguo («¿y ahora qué?», «ok») NO es un si: se pregunta explicitamente',
     /EXCEPCIÓN: un mensaje ambiguo o de impaciencia \("¿y ahora qué\?", "ok", "vale", "\.\.\."\)/.test(p) &&
     /NO es un "sí" a comprobar los requisitos ni una petición de empezar/.test(p));
comp('V18-B · lo que NO cambia: el salario NUNCA descarta, y el enrutado del Bloque 6 sigue igual',
     /EL SALARIO NUNCA DESCARTA/.test(p) && /- No dispone de la carta o documento de la empresa\./.test(p));
comp('V18-B18 · el declarante foral NO es señal de complejidad: la línea se va, queda solo la advertencia, y el v17 sí la tenía',
     !p.includes('- Declarante foral u otras particularidades.') && cuenta(p, 'Declarante foral') === 0 &&
     /SER DECLARANTE FORAL \(País Vasco o Navarra\) NO ES UNA SEÑAL DE COMPLEJIDAD/.test(p) && /NO lo mandes en senales_complejidad/.test(p) &&
     v17.includes('- Declarante foral u otras particularidades.'));

process.stdout.write(`\n${ok} verdes, ${mal} rojas\n`);
process.stdout.write(`v18 = ${car(p)} caracteres (v17 = ${car(v17)}, ${car(p) - car(v17) >= 0 ? '+' : ''}${car(p) - car(v17)})\n`);
