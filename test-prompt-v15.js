// Puerta del prompt v15 · 31/08/2026
//
...
const p   = fs.readFileSync(V15, 'utf8');   // EL QUE SE MIDE
...
// No se compara «que siga estando»: se compara BYTE A BYTE contra el v14.
{
  ...
  comp('N0 · el bloque de conocimiento fiscal es BYTE A BYTE el del v14',
       bloque15 === bloque14,
       'primera divergencia en el caracter ' + (() => {
         for (let i = 0; i < Math.max(bloque14.length, bloque15.length); i++) {
           if (bloque14[i] !== bloque15[i]) return i + ' → v14:' + JSON.stringify(bloque14.slice(i, i + 60)) + ' v15:' + JSON.stringify(bloque15.slice(i, i + 60));
         }
  ...
  comp('N0 · el disclaimer del bloque fiscal sigue en su sitio',
       /es\ninformación general sobre el régimen y no asesoramiento personalizado/.test(p) ||
       /información general sobre el régimen y no asesoramiento personalizado sobre su caso/.test(p));
}
...
process.stdout.write(`v15 = ${car(p)} caracteres (v14 = ${car(v14)}, ${car(p) - car(v14) >= 0 ? '+' : ''}${car(p) - car(v14)})\n`);
